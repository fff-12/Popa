...
class PasswordResetCode(models.Model):
    """
    Тимчасовий 6-значний код для відновлення пароля через email.
    Код дійсний 15 хвилин і може бути використаний лише раз.
    """
    ...
    def __str__(self):
        return f"{self.user.username} — {self.code} ({'використано' if self.is_used else 'активний'})"

    def is_valid(self):
        """Повертає True якщо код ще не використаний і не старіший за 15 хвилин."""
        if self.is_used:
            return False
        age = timezone.now() - self.created_at
        return age.total_seconds() < 900  # 15 хвилин

    @classmethod
    def generate_for(cls, user):
        """Інвалідує старі коди та генерує новий 6-значний код."""
